...
export default async function AdminPage({ params }: PageProps) {
  ...
  if (isAdmin) {
    return (
      <div className="min-h-screen flex flex-col max-w-5xl mx-auto px-6 py-12">
        <div className="flex flex-col space-y-8 bg-stone-50 border border-stone-200 p-8 rounded-xl shadow-xs">
          {/* Header */}
          <header className="flex justify-between items-baseline border-b border-stone-200 pb-4">
            ...
            <nav className="flex space-x-6 items-center text-xs font-medium">
              <Link href="/" className="text-stone-600 hover:text-accent transition-colors font-mono">
                {tNav('home')}
              </Link>
              <Link
                href="/admin"
                locale={otherLocale}
                className="px-2.5 py-1 rounded bg-stone-100 hover:bg-stone-200 transition-colors font-mono text-[10px] text-stone-700"
              >
                {tNav('langToggle')}
              </Link>
            </nav>
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-xl mx-auto px-6 py-12 justify-center">
      <div className="flex flex-col space-y-8 bg-white border border-stone-200 p-8 rounded-xl shadow-sm animate-scale-in">
        {/* Header */}
